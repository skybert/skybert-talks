...
I've been a programmer for 26 years and there are two problems that
...
Worked 17.5 years at [Stibo DX ](http://stibodx.com) aka Escenic,
making a Content Management System for the media industry.

---

## Customers from all over the world

- [The Economist](http://economist.com) in the UK
- [Thai PBS ](https://www.thaipbs.or.th) in ประเทศไทย
...
- What a character set is <!-- .element: class="fragment" -->
- What a character encoding is <!-- .element: class="fragment" -->
- Differentiate encoding problems from display problems <!-- .element: class="fragment" -->
- How to use UTF-8 everywhere <!-- .element: class="fragment" -->
...
- ...as long as you speak English  <!-- .element: class="fragment" -->

---

<img style="width: 800px" src="usa-uk.png" alt="usa &amp; uk"/>
...
<img src="columbus.png" alt="Columbus" style="height: 400px"/>
...
- Code pages; Windows-1250, Windows-1252, CP437 (IBM PC DOS)
...
func main() {
  ...
  fmt.Printf(
      "Character %v has code point %v and uses %v bytes\n",
      s,
      codePoint,
      size)
}
...
```java
String s = "🐘";
int unicodeCodepoint =  s.CodePointAt(0);
```
---
...
> One char ghost: 4  <!-- .element: class="fragment" -->
...
> One char ghost: 1  <!-- .element: class="fragment" -->

---

## Iterate through all code points in string
...
for i, r := range s {
	fmt.Printf(
      "Byte %d has code point %v U+%X %q\n",
      i, r, r, r)
	fmt.Printf(
      "Code point %v is encoded using %v bytes\n",
      r, utf8.RuneLen(r))
}
...
public void ghostIsOneCodeUnit() {
  final String ghost = "👻";
  assertEquals("One char ghost", 1, ghost.length());
}
```
...
Failed tests: ghostIsOneCodeUnit(GhostLengthFailingTest):
  One char ghost expected:<1> but was:<4>
```
---
...
- Many databases in Europe used, and several still use, [ISO-8859-1](http://no.wikipedia.org/wiki/ISO_8859-1) encoding.
...
- the database will throw an ERROR up to the web application.  <!-- .element: class="fragment" -->
